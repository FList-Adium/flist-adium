...
void flist_purple_set_status(PurpleAccount *account, PurpleStatus *status)
{
    ...
    char *statusMessage;
    statusMessage = purple_status_get_attr_string(status, FLIST_STATUS_MESSAGE_KEY);
    if(!statusMessage)
        statusMessage = "";
    ...
    {
        flist_set_status(fla, FLIST_STATUS_IDLE, statusMessage);
    }
    ...
    {
        flist_set_status(fla, fStatus, 	statusMessage);
    }else{
        ...
        switch (purple_status_type_get_primitive(statusType)) {
            ...
            case PURPLE_STATUS_EXTENDED_AWAY:
                flist_set_status(fla, FLIST_STATUS_AWAY, statusMessage);
                break;
            case PURPLE_STATUS_UNAVAILABLE:
                flist_set_status(fla, FLIST_STATUS_DND, statusMessage);
                break;
                
                // Assume AVAILABLE by default if it's not an AWAY/DND status
            default:
                flist_set_status(fla, FLIST_STATUS_AVAILABLE, statusMessage);
        }
    }
